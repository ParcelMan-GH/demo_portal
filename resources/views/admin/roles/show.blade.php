...
<div class="space-y-6" x-data="roleShowPage" data-role-show-config='@json($roleShowConfig)'>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 bg-slate-50/80 px-5 py-2">
            <div class="flex items-center gap-2 overflow-x-auto">
                <button type="button" @@click="setTab('permissions')" class="inline-flex items-center gap-2 whitespace-nowrap rounded-2xl px-4 py-2 text-sm font-semibold transition-all duration-200" :class="activeTab === 'permissions' ? 'bg-slate-800 text-white shadow-sm' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 hover:text-slate-800'">
                    Permissions
                    <span class="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-xs font-bold" :class="activeTab === 'permissions' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'">{{ $scopedPermissions->count() }}</span>
                </button>

                <button type="button" @@click="setTab('users')" class="inline-flex items-center gap-2 whitespace-nowrap rounded-2xl px-4 py-2 text-sm font-semibold transition-all duration-200" :class="activeTab === 'users' ? 'bg-slate-800 text-white shadow-sm' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-100 hover:text-slate-800'">
                    Assigned Users
                    <span class="inline-flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-xs font-bold" :class="activeTab === 'users' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'">{{ $assignedUsers->count() }}</span>
                </button>
            </div>
        </div>
