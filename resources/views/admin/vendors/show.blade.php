                    ...
                    <div class="flex items-start gap-5 lg:flex-shrink-0">
                        ...
                        <div class="space-y-2 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $vendor->is_phone_verified ? 'bg-blue-500/20 text-blue-300' : 'bg-amber-500/20 text-amber-300' }}">
                                    {{ $vendor->is_phone_verified ? 'Verified' : 'Pending' }}
                                </span>
                ...
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-5">
                    <div class="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                        ...
                        >
                            ...
                            <option value="forgot_pin">Forgot PIN</option>
                            <option value="reset_pin">Reset PIN</option>
                            <option value="profile_updated">Profile Updated</option>
                            <option value="pin_change_requested">PIN Change Requested</option>
                            <option value="pin_changed">PIN Changed</option>
                            <option value="resend_otp">Resend OTP</option>
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span
                                                ...
                                                :class="{
                                                    ...
                                                    'bg-blue-100 text-blue-700': ['profile_updated', 'pin_changed'].includes(log.action),
                                                    'bg-amber-100 text-amber-700': ['forgot_pin', 'reset_pin', 'pin_change_requested', 'resend_otp'].includes(log.action),
                                                    'bg-slate-100 text-slate-700': log.action === 'logout',
                                                    'bg-rose-100 text-rose-700': log.action === 'login_failed'
                                                }"
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <span
                                                ...
                                                :class="{
                                                    ...
                                                    'bg-amber-100 text-amber-700': log.purpose === 'pin_reset',
                                                    'bg-violet-100 text-violet-700': log.purpose === 'pin_change'
                                                }"
            ...
            >
                ...
                <form @@submit.prevent="saveVendor()">
                    <div class="space-y-5 px-6 py-6 max-h-[calc(100vh-240px)] overflow-y-auto">
                        ...
                        <div class="bg-gradient-to-br from-blue-50/50 to-indigo-50/30 rounded-2xl p-5 border border-blue-100/50">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-slate-800">Security PIN</h4>
                                    <p class="text-xs text-slate-500">Leave blank to keep current PIN</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- PIN -->
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-2">New PIN</label>
                                    <input
                                        type="text"
                                        x-model="form.pin"
                                        inputmode="numeric"
                                        pattern="[0-9]{4}"
                                        class="w-full px-4 py-2.5 border-2 border-blue-200 rounded-xl bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm text-slate-900 placeholder-slate-400 transition-all font-mono tracking-widest"
                                        placeholder="****"
                                        maxlength="4"
                                    >
                                </div>
                                ...
                                <!-- Confirm PIN -->
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-2">Confirm PIN</label>
                                    <input
                                        type="text"
                            ...
                            <template x-if="errors.pin">
                                <p class="mt-2 text-xs text-rose-600" x-text="errors.pin[0]"></p>
                            </template>
                        </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    <span
                                        ...
                                        class="pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow-lg ring-0 transition duration-300 ease-in-out flex items-center justify-center"
                                    >
                                        <svg x-show="form.is_active" class="w-3 h-3 text-emerald-600" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                                        </svg>
                                        <svg x-show="!form.is_active" x-cloak class="w-3 h-3 text-slate-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                                        </svg>
                                    </span>
                                </button>
    ...
    return {
        ...
        async saveVendor() {
            ...
            try {
                ...
                this.vendor.phone = this.form.phone;
