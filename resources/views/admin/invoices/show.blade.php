...
@section('breadcrumb-current', $invoice->invoice_number)

@php
$invoiceConfig = [
    'invoice' => $invoice,
    'downloadEndpoint' => route('admin.invoices.download', $invoice),
        ...
        <div class="relative">
            ...
            <div class="relative px-6 lg:px-8 py-6">
                ...
                <div class="flex items-center justify-between mb-6">
                    <a href="{{ route('admin.invoices.index') }}" class="group inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 text-white text-sm font-medium transition-all backdrop-blur-sm hover:shadow-md">
                        <svg class="w-4 h-4 transition-transform group-hover:-translate-x-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                        <span class="text-xs">Back to Invoices</span>
                    </a>

                    <div class="flex items-center gap-2">
                        <!-- Download PDF -->
                        <a href="{{ route('admin.invoices.download', $invoice) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white text-xs font-semibold rounded-xl border border-white/20 transition-all backdrop-blur-sm">
                        ...
                        <div class="space-y-2 min-w-0">
                            <div>
                                ...
                                <p class="text-slate-400 text-sm mt-0.5">
                                    Shipment:
                                    <a href="{{ route('admin.shipments.show', $invoice->shipment->id) }}" class="text-blue-400 hover:text-blue-300 hover:underline">
                        ...
                        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                            ...
                            <div class="p-5 space-y-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <a href="{{ route('admin.shipments.show', $invoice->shipment->id) }}" class="text-xs font-bold text-blue-600 hover:text-blue-800 hover:underline">
                                        {{ $invoice->shipment->shipment_number }}
                                    </a>
                                </div>
                                @if($invoice->shipment->vendor)
                                ...
                                <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                    ...
                                    <div class="flex flex-wrap items-center gap-3">
                                        ...
                                        <div class="text-xs font-medium text-slate-600">
                                            Page
                                            <span x-text="paymentPage"></span>
                                            of
                                            <span x-text="paymentLastPage()"></span>
                                        </div>
                                        ...
                                        <div class="flex space-x-1">
                                            ...
                                            <button
                                                @@click="paymentPage = Math.max(1, paymentPage - 1)"
                                                :disabled="paymentPage === 1"
                                                :class="paymentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/80'"
                                                class="w-7 h-7 border border-slate-200/70 rounded-lg bg-white/50 text-slate-600 flex items-center justify-center transition-colors"
                                            >
                                            ...
                                            <button
                                                @@click="paymentPage = Math.min(paymentLastPage(), paymentPage + 1)"
                                            ...
                                            <button
                                                @@click="paymentPage = paymentLastPage()"
                                                :disabled="paymentPage >= paymentLastPage()"
                                                :class="paymentPage >= paymentLastPage() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/80'"
                                                class="w-7 h-7 border border-slate-200/70 rounded-lg bg-white/50 text-slate-600 flex items-center justify-center transition-colors"
                                            >
...
function invoiceShow() {
    return {
        ...
        init() {
            ...
            // Auto-load payments summary for hero cards
            this.loadPayments();
        },
