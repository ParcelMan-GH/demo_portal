...
@section('breadcrumb-current', 'Create Shipment')
            ...
            <div class="relative px-6 py-5">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-lg font-bold text-white tracking-tight">Create Shipment</h1>
                            <p class="text-xs text-slate-400 mt-0.5">Create a new shipment on behalf of a vendor</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div x-show="step === 2" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-lg shadow-slate-200/50 p-6">
            <h2 class="text-base font-bold text-slate-900 flex items-center gap-2.5 mb-4">
                <span class="w-8 h-8 rounded-lg bg-violet-600 text-white flex items-center justify-center text-xs font-bold">2</span>
                Delivery Mode
            </h2>
            <div class="grid grid-cols-2 gap-4">
                ...
                <label class="cursor-pointer">
                    ...
                    <div class="relative p-4 rounded-2xl border-2 transition-all duration-200 peer-checked:border-amber-500 peer-checked:bg-amber-50 peer-checked:shadow-md peer-checked:shadow-amber-500/15 border-slate-200 hover:border-slate-300 hover:bg-slate-50">
                        ...
                        <p class="text-[11px] text-slate-500 mt-1">Each item has its own recipient &amp; fulfillment type</p>
...
function walkinShipment() {
    return {
        ...
        // Step navigation helpers — skip fulfillment step (3) when per_item
        // Internal step numbers: 1=Vendor, 2=Destination, 3=Fulfillment(single only), 4=Items, 5=Review
        // For per_item: 1=Vendor, 2=Destination, 3=Items, 4=Review (step 3 fulfillment is skipped)
        stepLabels() {
        ...
        stepMap() {
            // Maps visual stepper index → internal step number
            return this.destinationMode === 'single'
                ? [1, 2, 3, 4, 5]
                : [1, 2, 3, 4]; // 3=items(internally skipping fulfillment), 4=review
        },
