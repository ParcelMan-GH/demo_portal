{{--
    Packages editor partial — formerly the body of admin/shipments/edit.blade.php.
    Embedded inside the Packages tab on admin/shipments/show.blade.php. Runs in
    its own Alpine scope (shipmentEditor) so it doesn't clash with the outer
    shipmentShow component.

    Context required from caller: $editConfig (from ShipmentController::buildEditConfig).
--}}
<div x-data="shipmentEditor()" x-init="init()" data-edit-config='@json($editConfig)'>

    {{-- Hero and page chrome are provided by the parent show page. This
         partial only owns the interactive editor layout below. --}}
