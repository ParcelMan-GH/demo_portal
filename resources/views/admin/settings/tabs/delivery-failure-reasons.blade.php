...
<style>
    .delivery-failure-reason-form-row { display: grid; gap: 1rem; }
    @media (min-width: 768px) {
        .delivery-failure-reason-form-row { grid-template-columns: repeat(2, minmax(0, 1fr)); align-items: start; }
        .delivery-failure-reason-form-row > div:first-child { grid-column: 1 / -1; }
    }
</style>

<div x-data="deliveryFailureReasonsManager(@js($tabData['reasons'] ?? []), @js($tabData['types'] ?? []))" @@delivery-failure-reason-create.window="openCreate()" class="space-y-4">
            ...
            <table class="min-w-[900px] w-full divide-y divide-slate-200/50 text-xs">
                <thead class="bg-slate-50/70">
                    <tr>
                        ...
                        <th x-show="visibleColumns.type" class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500">Type</th>
                ...
                <tbody class="divide-y divide-slate-100/50">
                    <template x-if="pagedRows().length === 0">
                        <tr>
                            <td colspan="{{ $canEditSettings ? 5 : 4 }}" class="px-4 py-10 text-center">
                                <p class="text-sm font-medium text-slate-500">No reasons match your search</p>
                            </td>
                        </tr>
                    </template>
                    <template x-for="reason in pagedRows()" :key="reason.id">
                        <tr class="hover:bg-slate-50/70">
                            ...
                            <td x-show="visibleColumns.type" class="px-4 py-3">
                                <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-bold text-slate-700" x-text="typeLabel(reason.type)"></span>
                            </td>
                ...
                <div class="relative w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-2xl">
                    <div class="border-b border-slate-200 bg-slate-50/80 px-6 py-5">
                        <div class="flex items-start gap-4">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-600 text-white shadow-lg shadow-orange-600/20">
                                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M8 10h8M8 14h5m8-2a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                            </div>
                            <div class="min-w-0 flex-1">
                                <h3 class="text-xl font-black text-slate-950" x-text="editingId ? 'Edit Delivery Failure Reason' : 'Add Delivery Failure Reason'"></h3>
                                <p class="mt-1 text-sm font-semibold text-slate-500">Reasons appear in rider follow-ups and public issue reporting.</p>
                            </div>
                    ...
                    <form @@submit.prevent="save()">
                        <div class="space-y-4 p-6">
                            <div class="delivery-failure-reason-form-row">
                                <div>
                                    <label class="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Reason</label>
                                    <input x-model="form.label" required maxlength="120" class="w-full rounded-2xl border-2 border-slate-200 bg-white px-4 py-3 text-base font-bold text-slate-950 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100" placeholder="Recipient says not received">
                                </div>
                                <div>
                                    <label class="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Type</label>
                                    <select x-model="form.type" required class="w-full rounded-2xl border-2 border-slate-200 bg-white px-4 py-3 text-base font-bold text-slate-950 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100">
                            ...
                            <label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                                <span>
                                    ...
                                    <span class="block text-xs font-semibold text-slate-500">Show this reason in rider and public issue forms.</span>
                                </span>
        ...
        <template x-teleport="body">
            <div x-show="confirmModalOpen" x-cloak class="fixed inset-0 z-[130] flex min-h-screen w-screen items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" style="display:none" @@keydown.escape.window="closeConfirm()">
                <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl" @@click.outside="closeConfirm()">
                    <div class="px-6 py-5">
                        <h3 class="text-base font-black text-slate-900">Delete Delivery Failure Reason</h3>
                        <p class="mt-1 text-sm leading-6 text-slate-600">This removes <span class="font-bold text-slate-900" x-text="deleteTarget?.label || 'this reason'"></span> from future forms. Existing records keep their saved reason.</p>
                    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('deliveryFailureReasonsManager', (initialReasons = [], initialTypes = []) => ({
        reasons: initialReasons,
        types: initialTypes.length ? initialTypes : ['not_received', 'issue', 'failed', 'other'],
        modalOpen: false,
        ...
        visibleColumns: { reason: true, type: true, order: true, status: true },
        form: { label: '', type: 'issue', sort_order: 1, is_active: true },

        typeLabel(type) { return String(type || '').replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()); },
        ...
        filteredRows() {
            ...
            return this.sortedRows().filter((reason) => !search
                ...
                || String(reason.type || '').toLowerCase().includes(search)
        ...
        exportCsv() {
            const rows = [['Reason', 'Type', 'Order', 'Status'], ...this.filteredRows().map((reason) => [reason.label, this.typeLabel(reason.type), reason.sort_order || 0, reason.is_active ? 'Active' : 'Inactive'])];
            const csv = rows.map((row) => row.map((cell) => `"${String(cell ?? '').replace(/"/g, '""')}"`).join(',')).join('\n');
            const link = document.createElement('a');
            link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' }));
            link.download = 'delivery-failure-reasons.csv';
            link.click();
            URL.revokeObjectURL(link.href);
        },
        printTable() {
            const rows = this.filteredRows().map((reason) => [reason.label, this.typeLabel(reason.type), reason.sort_order || 0, reason.is_active ? 'Active' : 'Inactive']);
            const win = window.open('', '_blank');
            if (!win) return;
            win.document.body.innerHTML = `<style>body{font-family:system-ui,sans-serif;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #e2e8f0;padding:8px 12px;text-align:left;font-size:11px}th{background:#f1f5f9}</style><h1>Delivery Failure Reasons</h1><table><thead><tr><th>Reason</th><th>Type</th><th>Order</th><th>Status</th></tr></thead><tbody>${rows.map((row) => `<tr>${row.map((cell) => `<td>${cell || '-'}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
            win.document.close();
            win.print();
        },
        resetForm() {
            this.editingId = null;
            this.form = { label: '', type: 'issue', sort_order: (Math.max(0, ...this.reasons.map((r) => Number(r.sort_order || 0))) + 1), is_active: true };
        },
        openCreate() { this.resetForm(); this.modalOpen = true; },
        openEdit(reason) {
            this.editingId = reason.id;
            this.form = { label: reason.label || '', type: reason.type || 'issue', sort_order: Number(reason.sort_order || 0), is_active: !!reason.is_active };
            this.modalOpen = true;
        },
        ...
        async save() {
            ...
            try {
                const endpoint = this.editingId
                    ? window.settingsConfig.deliveryFailureReasonsUpdateEndpoint.replace('__ID__', this.editingId)
                    : window.settingsConfig.deliveryFailureReasonsStoreEndpoint;
        ...
        async toggle(reason) {
            this.saving = true;
            try {
                const res = await fetch(window.settingsConfig.deliveryFailureReasonsToggleEndpoint.replace('__ID__', reason.id), {
                    method: 'PATCH',
        ...
        async deleteReason() {
            ...
            this.saving = true;
            try {
                const res = await fetch(window.settingsConfig.deliveryFailureReasonsDeleteEndpoint.replace('__ID__', this.deleteTarget.id), {
                    method: 'DELETE',
                    headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': window.settingsConfig.csrfToken },
                });
                const data = await res.json().catch(() => ({}));
