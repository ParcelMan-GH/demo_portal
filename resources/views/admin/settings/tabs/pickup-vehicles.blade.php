...
<div x-data="pickupVehiclesManager(@js($tabData['vehicleTypes'] ?? []))" @@pickup-vehicle-create.window="openCreate()" class="space-y-4">
    <div class="pb-1">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-[980px] w-full divide-y divide-slate-200/50 text-xs">
                <thead class="bg-slate-50/70">
                    <tr>
                        <th x-show="visibleColumns.vehicle" class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500">Vehicle</th>
                        <th x-show="visibleColumns.capacity" class="px-4 py-3 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500">Capacity Hint</th>
                ...
                <tbody class="divide-y divide-slate-100/50">
                    <template x-if="pagedRows().length === 0">
                        <tr>
                            <td colspan="{{ $canEditSettings ? 6 : 5 }}" class="px-4 py-10 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-medium text-slate-500">No pickup vehicles match the current filters</p>
                                </div>
                            </td>
                        </tr>
                    </template>
                    <template x-for="type in pagedRows()" :key="type.id">
                        <tr class="hover:bg-slate-50/70">
                            <td x-show="visibleColumns.vehicle" class="whitespace-nowrap px-4 py-3">
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-slate-900" x-text="type.name"></p>
                                    <p class="mt-0.5 text-[11px] font-semibold text-slate-400" x-text="type.slug || '-'"></p>
                                </div>
                            </td>
                            <td x-show="visibleColumns.capacity" class="px-4 py-3 text-sm font-medium text-slate-600">
                                <p class="max-w-md truncate" x-text="type.capacity_hint || '-'"></p>
                            </td>
                            <td x-show="visibleColumns.order" class="whitespace-nowrap px-4 py-3 text-center text-sm font-bold text-slate-700" x-text="type.sort_order || 0"></td>
                            <td x-show="visibleColumns.status" class="whitespace-nowrap px-4 py-3 text-center">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold" :class="type.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'" x-text="type.is_active ? 'Active' : 'Inactive'"></span>
                            </td>
                            @if($canEditSettings)
                                <td class="whitespace-nowrap px-4 py-3 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        <button type="button" @@click="openEdit(type)" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition hover:bg-slate-50">Edit</button>
                                        <button type="button" @@click="toggle(type)" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold transition hover:bg-slate-50" :class="type.is_active ? 'text-amber-700' : 'text-emerald-700'" x-text="type.is_active ? 'Disable' : 'Enable'"></button>
                                        <button type="button" @@click="remove(type)" class="rounded-lg border border-rose-100 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 transition hover:bg-rose-100">Delete</button>
                                    </div>
                                </td>
                            @endif
                        </tr>
            ...
            <div x-show="modalOpen" x-cloak class="fixed inset-0 z-[120] flex min-h-screen w-screen items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" style="display:none">
                ...
                <div class="relative w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-2xl">
                    <div class="border-b border-slate-200 bg-slate-50/80 px-6 py-5">
                        <div class="flex items-start gap-4">
                            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-600 text-white shadow-lg shadow-orange-600/20">
                                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M8 17h8M6 17H5a2 2 0 01-2-2V7a2 2 0 012-2h9v12H8m8 0h3a2 2 0 002-2v-3.5a2 2 0 00-.4-1.2l-1.8-2.4A2 2 0 0017.2 7H14"/></svg>
                            </div>
                            <div class="min-w-0 flex-1">
                                <h3 class="text-xl font-black text-slate-950" x-text="editingId ? 'Edit Pickup Vehicle' : 'Add Pickup Vehicle'"></h3>
                                <p class="mt-1 text-sm font-semibold text-slate-500">Configure optional pickup vehicle choices for vendor requests.</p>
                            </div>
                    ...
                    <form @@submit.prevent="save()" class="space-y-4 p-6">
                        <div class="pickup-vehicle-form-row">
                            <div>
                                <label class="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Name</label>
                                <input x-model="form.name" required maxlength="80" class="w-full rounded-2xl border-2 border-slate-200 bg-white px-4 py-3 text-base font-bold text-slate-950 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100" placeholder="Motorbike">
                            </div>
                            <div>
                        ...
                        <div>
                            <label class="mb-2 block text-xs font-black uppercase tracking-wide text-slate-500">Capacity Hint</label>
                            <textarea x-model="form.capacity_hint" maxlength="180" rows="3" class="w-full resize-none rounded-2xl border-2 border-slate-200 bg-white px-4 py-3 text-base font-bold text-slate-950 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100" placeholder="Good for bulky local pickup loads."></textarea>
                        </div>
                        <label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            <span>
                                ...
                                <span class="block text-xs font-semibold text-slate-500">Show this vehicle option on the mobile app.</span>
                            </span>
                        ...
                        <div class="flex justify-end gap-3 border-t border-slate-100 pt-5">
                            ...
                            <button type="submit" :disabled="saving" class="rounded-2xl bg-orange-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-orange-600/20 transition hover:bg-orange-700 disabled:opacity-50" x-text="saving ? 'Saving...' : 'Save Vehicle'"></button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl" @@click.outside="closeConfirm()">
                    <div class="px-6 py-5">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="min-w-0">
                                <h3 class="text-base font-black text-slate-900">Delete Pickup Vehicle</h3>
                                <p class="mt-1 text-sm leading-6 text-slate-600">
                                    This removes <span class="font-bold text-slate-900" x-text="deleteTarget?.name || 'this vehicle type'"></span> from new vendor requests. Existing shipments will keep the saved vehicle name.
                                </p>
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4">
                        ...
                        <button type="button" @@click="deleteVehicle()" :disabled="saving" class="rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-rose-700 disabled:opacity-50" x-text="saving ? 'Deleting...' : 'Delete Vehicle'"></button>
                    </div>
                </div>
            </div>
        </template>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('pickupVehiclesManager', (initialTypes = []) => ({
        vehicleTypes: initialTypes,
        modalOpen: false,
        ...
        columns: [
            ...
            { key: 'capacity', label: 'Capacity Hint' },
            { key: 'order', label: 'Order' },
            { key: 'status', label: 'Status' },
        ],
        visibleColumns: { vehicle: true, capacity: true, order: true, status: true },
        form: { name: '', capacity_hint: '', sort_order: 1, is_active: true },
        ...
        sortedRows() {
            return [...this.vehicleTypes].sort((a, b) => Number(b.id || 0) - Number(a.id || 0));
        },
        filteredRows() {
            const search = this.search.trim().toLowerCase();
            return this.sortedRows().filter((type) => {
                const matchesSearch = !search
                    || String(type.name || '').toLowerCase().includes(search)
                    || String(type.slug || '').toLowerCase().includes(search)
                    || String(type.capacity_hint || '').toLowerCase().includes(search);
        ...
        exportRows() {
            return this.filteredRows().map((type) => [
                type.name || '',
                type.capacity_hint || '',
                type.sort_order || 0,
        ...
        exportCsv() {
            ...
            const csv = [['Vehicle', 'Capacity Hint', 'Order', 'Status'], ...rows]
                .map((row) => row.map((cell) => `"${String(cell ?? '').replace(/"/g, '""')}"`).join(','))
                .join('\n');
            const link = document.createElement('a');
            link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
            link.download = 'pickup-vehicles.csv';
        ...
        printTable() {
            ...
            win.document.body.innerHTML = `<style>body{font-family:system-ui,sans-serif;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #e2e8f0;padding:8px 12px;text-align:left;font-size:11px}th{background:#f1f5f9}</style><h1>Pickup Vehicles</h1><table><thead><tr><th>Vehicle</th><th>Capacity Hint</th><th>Order</th><th>Status</th></tr></thead><tbody>${rows.map((row) => `<tr>${row.map((cell) => `<td>${cell || '-'}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
            setTimeout(() => win.print(), 250);
        },
        openCreate() {
            const nextOrder = this.vehicleTypes.length ? Math.max(...this.vehicleTypes.map((type) => Number(type.sort_order || 0))) + 1 : 1;
            this.editingId = null;
            this.form = { name: '', capacity_hint: '', sort_order: nextOrder, is_active: true };
            this.modalOpen = true;
        },
        openEdit(type) {
        ...
        async save() {
            ...
            try {
                const url = this.editingId
                    ? window.settingsConfig.pickupVehiclesUpdateEndpoint.replace('__ID__', this.editingId)
                    : window.settingsConfig.pickupVehiclesStoreEndpoint;
                ...
                if (!response.ok) throw new Error(data.message || 'Unable to save vehicle type.');
                const row = data.vehicle_type;
                const index = this.vehicleTypes.findIndex((type) => Number(type.id) === Number(row.id));
                if (index >= 0) this.vehicleTypes.splice(index, 1, row);
                else this.vehicleTypes.push(row);
                this.closeModal();
            } catch (error) {
                alert(error.message || 'Unable to save vehicle type.');
            } finally {
                this.saving = false;
            }
        },
        async toggle(type) {
            const response = await fetch(window.settingsConfig.pickupVehiclesToggleEndpoint.replace('__ID__', type.id), {
                method: 'PATCH',
                headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': window.settingsConfig.csrfToken },
            });
            const data = await response.json();
            if (!response.ok) return alert(data.message || 'Unable to update vehicle type.');
            const index = this.vehicleTypes.findIndex((row) => Number(row.id) === Number(type.id));
            if (index >= 0) this.vehicleTypes.splice(index, 1, data.vehicle_type);
        },
        async remove(type) {
        ...
        async deleteVehicle() {
            ...
            this.saving = true;
            try {
                const response = await fetch(window.settingsConfig.pickupVehiclesDeleteEndpoint.replace('__ID__', this.deleteTarget.id), {
                    method: 'DELETE',
                    headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': window.settingsConfig.csrfToken },
                });
                const data = await response.json();
                if (!response.ok) throw new Error(data.message || 'Unable to delete vehicle type.');
                this.vehicleTypes = this.vehicleTypes.filter((row) => Number(row.id) !== Number(this.deleteTarget.id));
                if (this.currentPage > this.lastPage()) this.currentPage = this.lastPage();
                window.showToast?.(data.message || 'Pickup vehicle type deleted.', 'success');
                this.confirmModalOpen = false;
                this.deleteTarget = null;
            } catch (error) {
                window.showToast?.(error.message || 'Unable to delete vehicle type.', 'error');
            } finally {
                this.saving = false;
            }
        },
