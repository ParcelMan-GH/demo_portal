    ...
    <script>
        ...
        const endpoints = [
            ...
            },
            {
                ...
                description: 'Update a shipment. For submitted (unprocessed) shipments, only destination_mode, pickup_town, sender_notes, and photos can be changed. For draft shipments, all fields are editable. Send as multipart/form-data when including photos.',
                auth: true,
                group: 'shipments',
                userType: 'vendor',
                bodyType: 'formdata',
                useFormInputs: true,
                urlParams: [
                    { name: 'shipment', type: 'dropdown', required: true, description: 'Select editable shipment', source: 'shipments?status[]=draft&status[]=submitted', labelField: 'shipment_number', valueField: 'id', onSelect: 'prefillShipmentData' }
                ],
                fields: [
                    { name: 'destination_mode', type: 'enum', required: false, description: 'One drop-off or multiple drop-offs', options: ['single', 'per_item'], labels: { single: 'One Drop-off', per_item: 'Multiple Drop-offs' } },
                    { name: 'pickup_town', type: 'string', required: false, description: 'Pickup area (free text)', example: 'Lapaz' },
                ...
                exampleResponses: {
                    '200': {
                        ...
                        data: {
                            shipment: {
                                id: 40,
                                shipment_number: 'PCM-2026-00040',
                                status: 'submitted',
                                destination_mode: 'single',
                                delivery_preference: 'deliver',
                                sender_notes: 'Updated notes for admin',
                                is_processed: false,
                                items_count: 1,
                                total_photos_count: 7,
                                can_edit: true,
                                can_edit_fields: ['destination_mode', 'pickup_town', 'sender_notes', 'photos'],
                                can_delete: true,
                                pickup: { contact_name: 'Tony', contact_phone: '+233542796510', location: { town: 'Lapaz' } },
                                items: [{ id: 55, description: null, quantity: 1, images: ['url1', 'url2', '...'] }]
                            }
                        }
                    },
                    '400': {
            ...
            },
            {
                ...
                description: 'Delete a shipment. Works for draft shipments and submitted shipments that have not been processed (no driver assigned, no invoice created).',
                auth: true,
                group: 'shipments',
                userType: 'vendor',
                urlParams: [
