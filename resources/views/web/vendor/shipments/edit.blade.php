...
@section('content')
<main class="mx-auto min-h-screen w-full max-w-6xl px-6 py-10" x-data="vendorShipmentFormPage()" data-mode="edit" data-shipment-id="{{ $shipmentId }}">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <p class="text-xs font-semibold uppercase tracking-[0.14em] text-orange-200">Vendor Portal</p>
            <h1 class="mt-1 text-3xl font-extrabold text-white">Edit Shipment</h1>
        </div>
        <div class="flex gap-3">
            ...
            <a href="{{ route('web.vendor.shipments.show', ['shipment' => $shipmentId]) }}" class="rounded-xl border border-slate-200/20 bg-slate-900/70 px-4 py-2 text-sm font-semibold text-slate-200 hover:bg-slate-800">
                Back to Shipment
            </a>
        </div>
    </div>
        ...
        <form class="space-y-6" @submit.prevent="saveShipment()">
            ...
            <div class="flex flex-wrap gap-3">
                <button type="submit" :disabled="saving"
                    ...
                    <span x-show="!saving">Update Shipment</span>
                    <span x-show="saving" x-cloak>Saving...</span>
                </button>
                <a :href="`/vendor/shipments/${shipmentId}`"
