...
>
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Pending Confirmations</h1>
            <p class="mt-1 text-sm font-semibold text-slate-500">Bus courier handoffs waiting for recipient delivery confirmation.</p>
        </div>
    </div>

    <!-- Unified Minimal Cream Metric Cards -->
    <div class="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <div class="relative flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-[#FFFCF8] p-5 shadow-sm">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-200/60 bg-white text-slate-400">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M9 12l2 2 4-4m6 2a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
            </div>
            <div class="min-w-0">
                <p class="truncate text-[10px] font-black uppercase tracking-widest text-slate-400">Pending</p>
                <p class="mt-0.5 text-2xl font-black text-slate-900" x-text="meta.total || 0">0</p>
            </div>
        </div>
        
        <div class="relative flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-[#FFFCF8] p-5 shadow-sm">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-200/60 bg-white text-slate-400">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M12 8v4m0 4h.01M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/></svg>
            </div>
            <div class="min-w-0">
                <p class="truncate text-[10px] font-black uppercase tracking-widest text-slate-400">Needs Follow-up</p>
                <p class="mt-0.5 text-2xl font-black text-slate-900" x-text="meta.needs_followup || 0">0</p>
            </div>
        </div>
        
        <div class="relative flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-[#FFFCF8] p-5 shadow-sm">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-200/60 bg-white text-slate-400">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M12 6v6l4 2m6-2a10 10 0 1 1-20 0 10 10 0 0 1 20 0Z"/></svg>
            </div>
            <div class="min-w-0">
                <p class="truncate text-[10px] font-black uppercase tracking-widest text-slate-400">Under 24h</p>
                <p class="mt-0.5 text-2xl font-black text-slate-900" x-text="meta.pending_recent || 0">0</p>
            </div>
        </div>
        
        <div class="relative flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-[#FFFCF8] p-5 shadow-sm">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-slate-200/60 bg-white text-slate-400">
                <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7" d="M20 7h-7m7 0v7m0-7-8 8-4-4-5 5"/></svg>
            </div>
            <div class="min-w-0">
                <p class="truncate text-[10px] font-black uppercase tracking-widest text-slate-400">This Page</p>
                <p class="mt-0.5 text-2xl font-black text-slate-900" x-text="stops.length || 0">0</p>
            </div>
        </div>
    </div>
            ...
            <div class="mb-4 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
                ...
                <div class="w-full xl:max-w-md">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            placeholder="Search run, town, or courier..."
                            class="w-full rounded-2xl border-2 border-slate-200 bg-white py-3 pl-12 pr-4 text-sm font-semibold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-orange-400 focus:ring-4 focus:ring-orange-100"
                        >
                    </div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3 xl:justify-end">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-cloak @click.away="open = false" x-transition class="absolute right-0 z-50 mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl" style="display: none;">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-cloak @click.away="open = false" x-transition class="absolute right-0 z-50 mt-2 w-44 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl" style="display: none;">
                            <button type="button" @click="exportData('csv'); open = false" class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-50">CSV</button>
                            <button type="button" @click="exportData('print'); open = false" class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-50">Print</button>
                        </div>
                    </div>
            ...
            <div class="hidden overflow-x-auto lg:block">
                <table class="w-full table-auto divide-y divide-slate-200/50 text-xs">
                    <thead class="bg-slate-50">
                        <tr>
                            <th x-show="visibleColumns.run_number" class="w-[13%] px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-slate-500">Run #</th>
                            <th x-show="visibleColumns.recipient" class="w-[19%] px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-slate-500">Recipient</th>
                            <th x-show="visibleColumns.destination" class="w-[18%] px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-slate-500">Destination</th>
                            <th x-show="visibleColumns.courier" class="w-[19%] px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-slate-500">Courier</th>
                            <th x-show="visibleColumns.handoff_at" class="w-[13%] px-4 py-3 text-left text-[10px] font-black uppercase tracking-wider text-slate-500">Handed Off</th>
                            <th x-show="visibleColumns.packages" class="w-[8%] px-3 py-3 text-center text-[10px] font-black uppercase tracking-wider text-slate-500">Packages</th>
                            <th x-show="visibleColumns.status" class="w-[10%] px-3 py-3 text-center text-[10px] font-black uppercase tracking-wider text-slate-500">Status</th>
                            <th x-show="visibleColumns.actions" class="px-4 py-3 text-right text-[10px] font-black uppercase tracking-wider text-slate-500">Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td :colspan="visibleColumnCount()" class="px-4 py-12 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        <div class="flex h-11 w-11 items-center justify-center rounded-full bg-slate-50 text-slate-400">
                        ...
                        <template x-for="stop in stops" :key="stop.id">
                                <tr class="hover:bg-orange-50/30 transition-colors">
                                    <td x-show="visibleColumns.run_number" class="whitespace-nowrap px-4 py-3">
                                        <a :href="stop.run_url" class="font-mono text-xs font-black text-slate-900 hover:text-orange-700 underline decoration-slate-300 underline-offset-4" x-text="stop.run_number || '-'"></a>
                                    </td>
                                    <td x-show="visibleColumns.recipient" class="px-4 py-3">
                                        <p class="font-bold text-slate-900" x-text="stop.recipient_name || '-'"></p>
                                        <p class="mt-1 font-mono text-[11px] font-semibold text-slate-500" x-text="stop.recipient_phone || '-'"></p>
                                    </td>
                                    <td x-show="visibleColumns.destination" class="px-4 py-3">
                                        ...
                                        <p class="mt-1 text-[11px] font-semibold text-slate-500" x-text="[stop.destination_district || stop.district, stop.region].filter(Boolean).join(', ') || '-'"></p>
                                    </td>
                                    <td x-show="visibleColumns.courier" class="px-4 py-3">
                                        <p class="font-semibold text-slate-900" x-text="stop.courier_name || '-'"></p>
                                        <p class="mt-1 font-mono text-[11px] font-semibold text-slate-500" x-text="stop.courier_phone || '-'"></p>
                                        <p class="mt-1 text-[10px] font-bold text-slate-400 uppercase" x-text="stop.vehicle_number || 'No vehicle'"></p>
                                    </td>
                                    <td x-show="visibleColumns.handoff_at" class="whitespace-nowrap px-4 py-3">
                                        ...
                                        <span class="mt-1.5 inline-flex rounded-full px-2 py-0.5 text-[10px] font-black" :class="stop.needs_followup ? 'bg-rose-50 text-rose-700 ring-1 ring-rose-200' : 'bg-slate-100 text-slate-600'" x-text="hoursLabel(stop)"></span>
                                    </td>
                                    ...
                                    <td x-show="visibleColumns.status" class="whitespace-nowrap px-3 py-3 text-center">
                                        <span class="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wider" :class="stop.needs_followup ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-amber-200 bg-amber-50 text-amber-700'" x-text="stop.needs_followup ? 'Needs Follow-up' : 'Pending'"></span>
                                    </td>
                                    <td x-show="visibleColumns.actions" class="whitespace-nowrap px-4 py-3 text-right">
                                        <div class="inline-flex items-center justify-end gap-2">
                                            <button type="button" @click="openActionModal(stop, 'delivered')" class="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] font-black text-emerald-700 transition hover:bg-emerald-100">Confirm Delivery</button>
                                            <button type="button" @click="openActionModal(stop, 'failed')" class="rounded-xl border border-rose-200 bg-white px-3 py-2 text-[11px] font-black text-rose-700 transition hover:bg-rose-50">Mark Failed</button>
                                        </div>
                                    </td>
                                </tr>
                        </template>
            ...
            <div class="border-t border-slate-100 bg-slate-50/70 px-4 py-3">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-1.5">
                        <button @click="goToPage(currentPage - 1)" :disabled="currentPage <= 1" class="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                        <div class="px-2 text-xs font-extrabold text-slate-700">Page <span x-text="meta.current_page || 1"></span> / <span x-text="meta.last_page || 1"></span></div>
                        <button @click="goToPage(currentPage + 1)" :disabled="currentPage >= meta.last_page" class="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40">
    ...
    <template x-teleport="body">
        <div
            ...
            class="fixed inset-0 z-[120] flex min-h-screen items-center justify-center bg-slate-900/60 px-4 py-6 backdrop-blur-sm sm:p-4"
            style="display: none;"
        ...
        >
            <div class="relative flex max-h-[calc(100dvh-3rem)] w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl sm:max-h-[calc(100vh-2rem)]" @click.stop>
                <div class="shrink-0 flex items-start justify-between border-b border-slate-100 p-6">
                    <div class="flex items-start gap-4">
                        <div
                            class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-white shadow-lg"
                            :class="actionType === 'failed' ? 'bg-rose-600 shadow-rose-600/20' : 'bg-emerald-600 shadow-emerald-600/20'"
                        >
                            <svg x-show="actionType !== 'failed'" class="h-7 w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="min-w-0 mt-0.5">
                            <h3 class="text-xl font-black text-slate-900" x-text="actionType === 'failed' ? 'Mark Delivery Failed' : 'Confirm Delivery'"></h3>
                            <p class="mt-1 text-sm font-semibold text-slate-500" x-text="actionType === 'failed' ? 'Record that the recipient did not receive this handoff.' : 'Confirm the recipient received this handoff.'"></p>
                        </div>
                    </div>
                    <button type="button" @click="closeActionModal()" class="shrink-0 rounded-2xl border border-slate-200 p-2.5 text-slate-400 transition hover:bg-slate-50 hover:text-slate-700">
                ...
                <div class="min-h-0 flex-1 space-y-5 overflow-y-auto bg-slate-50/70 p-6">
                    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <p class="text-[10px] font-black uppercase tracking-wide text-slate-400">Recipient</p>
                        <div class="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-3">
                            <div class="min-w-0">
                                <p class="truncate text-base font-black text-slate-900" x-text="actionStop?.recipient_name || '-'"></p>
                                <p class="mt-0.5 font-mono text-sm font-semibold text-slate-500" x-text="actionStop?.recipient_phone || '-'"></p>
                            </div>
                            <span class="shrink-0 rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-700" x-text="(actionStop?.packages_count || actionStop?.total_packages || 0) + ' package(s)'"></span>
                        </div>
                        <div class="mt-3 grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
                            <div>
                                ...
                                <p class="mt-1 font-bold text-slate-800" x-text="actionStop?.courier_name || '-'"></p>
                            </div>
                            <div>
                                <p class="text-[10px] font-black uppercase tracking-wide text-slate-400">Handed Off</p>
                                <p class="mt-1 font-bold text-slate-800" x-text="actionStop?.handed_off_at || actionStop?.handoff_at || '-'"></p>
                            </div>
                        </div>
                    </div>

                    <div>
                        <label class="mb-2 block text-xs font-black uppercase tracking-wide text-slate-600">Notes <span class="font-semibold normal-case tracking-normal text-slate-400">(optional)</span></label>
                        <textarea
                            x-model="actionNotes"
                            rows="3"
                            placeholder="Add confirmation or failure notes..."
                            class="w-full resize-none rounded-xl border-2 border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-orange-400 focus:ring-4 focus:ring-orange-100"
                        ></textarea>
                    </div>
                </div>

                <div class="shrink-0 flex justify-end gap-3 rounded-b-3xl border-t border-slate-100 bg-white p-5">
                    <button type="button" @click="closeActionModal()" class="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50">Cancel</button>
                    <button
                        type="button"
                        @click="submitAction(actionStop, actionType)"
                        :disabled="submitting || !actionStop"
                        class="rounded-xl px-6 py-3 text-sm font-black text-white shadow-lg transition disabled:cursor-not-allowed disabled:opacity-50"
                        :class="actionType === 'failed' ? 'bg-rose-600 shadow-rose-600/20 hover:bg-rose-700' : 'bg-emerald-600 shadow-emerald-600/20 hover:bg-emerald-700'"
                    >
                        <span x-text="submitting ? 'Saving...' : (actionType === 'failed' ? 'Mark Failed' : 'Confirm Delivered')"></span>
                    </button>
                </div>
            </div>
        </div>
...
function pendingConfirmations() {
    ...
    return {
        ...
        statusBadgeClass(status) {
            return status === 'needs_followup' ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-amber-200 bg-amber-50 text-amber-700';
        },
