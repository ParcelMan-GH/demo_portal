...
<div class="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 space-y-6"
    ...
    <!-- Page Header (Positioned Above the Cards) -->
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Users & Staff Hub</h1>
        ...
        <div class="flex items-center gap-4">
            <span class="inline-flex items-center rounded-full bg-white border border-slate-200/80 px-4 py-2.5 text-sm font-black text-slate-700 shadow-sm" x-text="meta.total + ' users'"></span>
            
            @if($canCreateUsers)
            <button @click="openCreateModal()" class="bg-[#E2762B] hover:bg-[#d1651d] text-white font-bold text-sm px-6 py-3 rounded-2xl shadow-md transition-colors flex items-center gap-2">
    ...
    <div class="relative flex flex-col gap-6 lg:flex-row lg:items-start min-h-[760px]">
        
        <!-- Loading Overlay -->
        <div x-show="loading" x-transition.opacity.duration.150ms class="absolute inset-0 z-10 bg-slate-50/60 backdrop-blur-[1px] rounded-3xl" style="display: none;"></div>

        <!-- LEFT SIDE: Worker Master Roster (33% width) -->
        <div class="w-full lg:w-4/12 shrink-0 flex flex-col rounded-3xl border border-slate-200/80 bg-white shadow-lg shadow-slate-300/30 overflow-hidden min-h-[720px]">
            ...
            <div class="border-b border-slate-100 bg-slate-50/70 p-5 space-y-4">
                ...
                <div x-show="showFilters" x-transition class="space-y-3 pt-3 border-t border-slate-200/60" style="display:none">
                    <select x-model="roleFilter" @change="applyFilters()" class="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-800 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
        ...
        <div class="w-full lg:w-8/12 flex-1 flex flex-col rounded-3xl border border-slate-200/90 bg-white shadow-lg shadow-slate-300/30 overflow-hidden min-h-[720px]">
            ...
            <div x-show="selectedUser" class="flex flex-col h-full" style="display: none;">
                ...
                <div class="relative overflow-hidden border-b border-slate-200/80 bg-gradient-to-br from-orange-50/70 via-white to-slate-50/60 p-8">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                        ...
                        <button type="button" @click="selectedUser = null" class="self-start rounded-2xl border border-slate-200 bg-white px-4 py-2 text-xs font-black text-slate-500 hover:bg-slate-50 hover:text-slate-800 transition shadow-sm">
                            Deselect &times;
                        </button>
                    </div>
                </div>
