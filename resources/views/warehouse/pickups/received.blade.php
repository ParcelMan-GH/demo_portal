...
@section('content')
<div class="space-y-6" x-data="warehouseReceivedPickupsPage" data-warehouse-received-pickups-config="{{ e(json_encode($config)) }}">
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h2 class="text-lg font-semibold text-slate-900">Received Pickups</h2>
                <p class="text-sm text-slate-500">Pickups already received at this warehouse.</p>
            </div>
        ...
        <div class="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex flex-1 flex-col gap-3 md:flex-row md:items-center">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button type="button" @@click="open = !open" class="inline-flex w-full md:w-48 items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50">
                    ...
                    <div x-show="open" x-cloak @@click.away="open = false" class="absolute z-30 mt-2 w-full rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                        ...
                        <template x-for="status in statuses" :key="status.value">
                            <button type="button" @@click="setStatusFilter(status.value, status.label); open = false" class="mt-1 w-full rounded-lg px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50" x-text="status.label"></button>
                        </template>
                    </div>
                </div>
            </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th x-show="visibleColumns.driver_name" @@click="sort('driver_name')" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">Driver</th>
                        <th x-show="visibleColumns.driver_phone" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">Driver Phone</th>
                        <th x-show="visibleColumns.status" @@click="sort('status')" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">Status</th>
                        <th x-show="visibleColumns.assigned_at" @@click="sort('assigned_at')" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">Assigned At</th>
                        <th x-show="visibleColumns.arrived_warehouse_at" @@click="sort('arrived_warehouse_at')" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">Arrived Warehouse At</th>
                        ...
                        <th x-show="visibleColumns.receive_notes" class="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500">Receive Notes</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    <tr x-show="!loading && rows.length === 0" x-cloak>
                        <td :colspan="visibleColumnCount()" class="px-4 py-10 text-center text-sm text-slate-500">No received pickups found.</td>
                    </tr>
                    <template x-for="row in rows" :key="row.id">
                        <tr class="hover:bg-slate-50/70">
                            ...
                            <td x-show="visibleColumns.driver_phone" class="px-4 py-3 text-slate-700" x-text="row.driver_phone || '-' "></td>
                            <td x-show="visibleColumns.status" class="px-4 py-3"><span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700" x-text="row.status || '-' "></span></td>
                            <td x-show="visibleColumns.assigned_at" class="px-4 py-3 text-slate-600" x-text="row.assigned_at || '-' "></td>
                            <td x-show="visibleColumns.arrived_warehouse_at" class="px-4 py-3 text-slate-600" x-text="row.arrived_warehouse_at || '-' "></td>
                            <td x-show="visibleColumns.received_at" class="px-4 py-3 text-slate-600" x-text="row.received_at || '-' "></td>
                            <td x-show="visibleColumns.receive_notes" class="px-4 py-3 text-slate-600" x-text="row.receive_notes || '-' "></td>
                        </tr>
                    </template>
                </tbody>
            </table>
