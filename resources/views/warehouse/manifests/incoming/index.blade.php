...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('warehouseIncomingManifestsPage', () => ({
            ...
            config: {},
            init() {
                const configAttr = this.$el.getAttribute('data-warehouse-incoming-manifests-config');
            ...
            startScanner() {
                ...
                this.scannerStatus = 'Camera active...';
            },
            scanIncomingPackage() {
                if (!this.scannerCode.trim()) return;
                this.scannerLoading = true;
                
                fetch(this.config.scan_endpoint, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    },
                    body: JSON.stringify({ code: this.scannerCode })
                })
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
                        ...
                        this.scanModalMessage = data.message || 'Invalid barcode.';
                    }
                })
                .finally(() => { this.scannerLoading = false; });
            },
            receiveBatch(batchId) {
                if (!confirm('Are you sure you want to receive and unseal this batch?')) return;

                fetch(`/warehouse/manifests/incoming/${batchId}/receive`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert(data.message || 'Failed to receive batch.');
                    }
                })
                .catch(() => alert('Error receiving batch.'));
            }
        }));
    });
</script>
